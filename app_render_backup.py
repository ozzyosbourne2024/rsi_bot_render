...
# AYARLAR
# =====================
SYMBOLS = {"ALTIN": "GC=F", "GUMUS": "SI=F", "NDX": "^NDX"}
RSI_PERIOD = 14
LAST_ALERT = {}
...
def check_alarm(name, rsi_val):
    ...
    if rsi_val < 30 and prev != "LOW": LAST_ALERT[name] = "LOW"; return f"🔴 {name} RSI < 30 ({rsi_val:.2f})"
    if 45 < rsi_val <= 50 and prev != "MID": LAST_ALERT[name] = "MID"; return f"🟠 {name} RSI 45–50 ({rsi_val:.2f})"
    if rsi_val > 50 and prev != "HIGH": LAST_ALERT[name] = "HIGH"; return f"🟢 {name} RSI > 50 ({rsi_val:.2f})"
    return None
...
def send_report():
    ...
    for name, symbol in SYMBOLS.items():
        data = fetch(symbol)
        if not data: text += f"{name}: Veri alınamadı!\n"; continue
...
# Çalıştırma
# =====================
send_report()  # Scheduled Job tarafından çağrılacak
